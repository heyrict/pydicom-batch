...
class SCP(object):
    ...
    def format_dicom_template(self, template, ds):
        """
        Format a template string with DICOM tag values.

        Parameters
        ----------
        template : str
            Template string with DICOM tag names in curly braces, e.g., '{PatientID}/{StudyInstanceUID}'
        ds : pydicom.dataset.Dataset
            DICOM dataset to extract values from

        Returns
        -------
        str
            Formatted string with DICOM values, 'UNK' for missing tags
        """
        import re
        ...
        for tag in tags:
            try:
                element = ds[ElementPath(tag).tag]
                value = str(element.value)
                values[tag] = re.sub(r"[^a-zA-Z0-9_\- ]", "_", value)
    ...
    def write_file(self, i, q):
        while True:
            ...
            dir_structure = self.format_dicom_template(
                self.config["output"]["directory_structure"], ds
            )
            filename = self.format_dicom_template(self.config["output"]["filename"], ds)
    ...
    def start_server(self):
        print(
            "Starting local storage SCP server on port {}".format(
        ...
        self.scp = self.ae.start_server(
            ("", self.config["local"]["port"]), block=False, evt_handlers=handlers
        )

    def stop_server(self):
        old_qsize = self.writing_queue.qsize()
        pbar = tqdm.tqdm(total=old_qsize, desc="Post-processing ", unit="files")
        ...
        print(
            "Stopping local storage SCP server: {} files transferred in {:.1f} seconds ({:.2f} files/s)".format(
                self.file_count, time_elapsed, self.file_count / time_elapsed
            )
        )
        self.scp.shutdown()
        temp_dir = os.path.join(self.config["output"]["directory"], "tmp")
    ...
    def handle_store(self, event):
        ...
        filename = os.path.join(
            self.config["output"]["directory"], "tmp/{0!s}.dcm".format(uuid.uuid4())
        )
