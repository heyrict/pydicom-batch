    ...
    if os.path.exists(filepath):
        with open(
            ...
            writer = csv.DictWriter(
                csvfile, fieldnames=sorted(fieldnames), dialect="excel"
            )
...
def add_element_to_list(element_list, key, value):
    match_elements = [
        x if ((x.partition("=")[0] == key) or (x == key)) else None
        for x in element_list
    ]
    if match_elements.count(None) != len(match_elements):
        theVal = [
            f"{key}={value}" if ((x.partition("=")[0] == key) or (x == key)) else x
            for x in element_list
        ]
        return theVal
    else:
...
def create_requests(config):
        ...
        if os.path.exists(config["request"]["elements_batch_file"]):
            with open(config["request"]["elements_batch_file"], newline="") as csvfile:
                ...
                for row in reader:
                    ...
                    for key in row:
                        row_request["elements"] = add_element_to_list(
                            row_request["elements"], key, row[key]
                        )
    ...
    filepath_requests_completed = os.path.join(
        config["output"]["directory"], "requests.completed"
    )
    filepath_requests_failed = os.path.join(
        config["output"]["directory"], "requests.failed"
    )
    ...
    if requests:
        ...
        if config["request"]["threads"] > 1:
            with concurrent.futures.ThreadPoolExecutor(
                max_workers=config["request"]["threads"]
            ) as executor:
...
class SCU(object):
                ...
                if sec_until_end > sec_until_start:
                    if self.pbar:
                        self.pbar.set_description(
                            "Extraction PAUSED (will resume at {})".format(
                                self.config["schedule"]["start_time"]
                            )
                        )
                    ...
                    if self.pbar:
                        self.pbar.set_description(
                            "Sending {} requests (will pause at {})".format(
                                self.config["request"]["type"],
                                self.config["schedule"]["end_time"],
                            )
                        )
                ...
                if "Status" in status:
                    if status and status.Status in [0xFF00, 0xFF01]:
                        ...
                        path = os.path.join(
                            ...
                            self.config["output"]["database_file"],
                        )
                        ...
                        if identifier.Status in [hex(0x0000)]:
                            filepath_requests_completed = os.path.join(
                                self.config["output"]["directory"], "requests.completed"
                            )
                            dict_to_csv(
                                request, filepath_requests_completed, request.keys()
                            )
                        else:
                            print("Failed with code ", hex(status.Status))
                            filepath_requests_failed = os.path.join(
                                self.config["output"]["directory"], "requests.failed"
                            )
                            dict_to_csv(
                                request, filepath_requests_failed, request.keys()
                            )
    ...
    def send_move(self, request):
        ...
        if self.association.is_established:
            responses = self.association.send_c_move(
                identifier, self.config["local"]["aet"], self.query_model
            )
